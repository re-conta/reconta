...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between gap-3">
      <p class="text-xs text-ink-500">Contas bancárias, cartões e investimentos</p>
      <button
        type="button"
        class="shrink-0 rounded-full bg-ink-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-ink-800"
        @click="startCreate"
      >
        + Nova conta
      </button>
    ...
    <form
      v-if="showForm"
      class="flex flex-col gap-4 rounded-2xl border border-ink-100 bg-ink-50/50 p-4"
    ...
    <div class="overflow-hidden rounded-2xl border border-ink-100">
      ...
      <p v-else-if="errorMessage" class="p-6 text-center text-sm text-coral-600">
        {{ errorMessage }}
      </p>
      <div v-else-if="accounts.length === 0" class="flex flex-col items-center gap-1 p-8 text-center">
      ...
      <ul v-else class="divide-y divide-ink-100">
        <li
          ...
          :key="account.id"
          class="flex items-center justify-between gap-3 px-4 py-3 transition hover:bg-ink-50/60"
        >
          <div class="min-w-0">
