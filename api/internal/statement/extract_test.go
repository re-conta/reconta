...
// buildTdLineMovesTestPDF monta um PDF cujas linhas são posicionadas com Td
// (movimento relativo) em vez de Tm (matriz absoluta) — reproduzindo o
// layout de extratos Sicredi mais recentes (jan/2026 em diante), cujo
// gerador de PDF passou a emitir as linhas dessa forma.
func buildTdLineMovesTestPDF(lines []string) []byte {
	var content strings.Builder
	content.WriteString("BT /F1 12 Tf 1 0 0 1 50 750 Tm\n")
...
func TestExtractHandlesTdLineMoves(t *testing.T) {
	pdfBytes := buildTdLineMovesTestPDF([]string{
	...
	if len(parsed) != 2 {
		t.Fatalf("esperava 2 lançamentos a partir de um PDF com linhas via Td, obteve %d a partir de %q: %+v", len(parsed), text, parsed)
	}
